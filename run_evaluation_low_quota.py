"""
Low-quota A/B evaluation runner for the RAG lab.

Run from repository root, for example:
    python run_evaluation_low_quota.py --limit 2 --out smoke_low --model gemini-2.5-flash-lite
    python run_evaluation_low_quota.py --limit 15 --out full_low --model gemini-2.5-flash-lite

Design:
- Config A: dense top-k
- Config B: dense + BM25 (2*top-k each) + RRF -> final top-k
- Exactly ONE Gemini request per evaluated case.
- That single request generates answers for A/B and judges four metrics for both.
- No Ragas calls, so this is an explicit LLM-as-judge evaluation, not Ragas.
- Resume is supported.
"""
